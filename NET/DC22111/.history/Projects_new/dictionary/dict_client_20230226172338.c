...
int main(int argc, char const *argv[])
{
    ...
    sin.sin_port = htons(IP); //服务器绑定的端口号
    sin.sin_addr.s_addr = inet_addr(PORT); //服务器绑定的IP
...
void do_register(int cfd)
{
    ...
    while (getchar()!=10);
    //2. 发送消息给服务器
    ssize_t sret = send(cfd,&msg,sizeof(msg),0);
    if (sret<0)
    {
        ERR_MS("client do_register send");
        return;
    }
    //3.接收服务器返回的消息
    ...
    if (rret<0)
    {
        ERR_MS("client do_register recv");
        return;
    }
    //4.返回的消息判断是否注册成功
    printf("%s\n",msg.text);
