...
int deal_cli_inter(int newfd,struct sockaddr_in cin,sqlite3 *db)
{
    ...
    while (1)
    {
        ...
        } else if (0 == ret)
        {
            ...
            printf("msg.type=%d\n",msg.type);
            printf("msg.text=%s\n",msg.text);
            if (NULL!=msg.name && 0!=strlen(msg.name))
            {
                char sql[128] = {0};
                msg.status = 0;
                sprintf(sql, "update user set status = %d where name = '%s'",msg.status, msg.name);
                printf("server ret0_exit sql = %s \n",sql);
                char *errmsg = NULL;
                if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
                {
                    fprintf(stderr,"server ret0_exit failed,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
                    return -1;
                }
                printf("%s server ret0_exit 用户设置退出状态成功\n",msg.name);
            }
...
int load_db(sqlite3 *db,char *filepath)
{
    ...
    char sql1[64] = "select * from dictionary";
    char **pres = NULL;
    int row,column;
    char *errmsg = NULL;
...
void do_server_register(int newfd, msg_t msg, sqlite3 *db)
{
    ...
    sprintf(sql,"insert into user values(\"%s\",\"%s\",%d)",msg.name,msg.passwd,msg.status);
    ...
    } else
    {
        ...
        ssize_t sret = send(newfd,&msg,sizeof(msg),0);
        if (sret<0)
        {
...
{
    char sql[256] = {0};
    sprintf(sql,"select * from dictionary where word = \"%s\"",msg.text) ;
    ...
    } else
    {
        ...
        sprintf(sql,"insert into history values(\"%s\",\"%s\",\"%s\",\"%s\")",msg.name,pres[2],pres[3],time_buf);
...
void do_server_history_query(int newfd,msg_t msg,sqlite3 * db)
{
    ...
    int row,column;
    char *errmsg = NULL;
    char sql[128] = "select * from history where name = \"%s\"";
