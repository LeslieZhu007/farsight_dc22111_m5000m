...
void do_server_register(int newfd, msg_t msg, sqlite3 *db)
{
    ...
    } else
    {
        ...
        if (sret<0)
        {
            ERR_MS("server do_server_register send");
            return;
        }
    } 
    return;
...
void do_server_login(int newfd, msg_t msg, sqlite3 *db)
{
    ...
    if (row <=0)
    {
        ...
        if (sret<0)
        {
            ERR_MS("server do_server_login send");
            return;
        }
    } else
    {
...
{
    char sql[128] = {0};
    sprintf(sql,"select * from dictionary where word = \"%s\"",msg.name,msg.text) ;
    ...
    if (row <=0)
    {
        ...
        if (sret<0)
        {
            ERR_MS("server do_server_word_query send");
            return;
        }
    } else
    {
    ...
    for (int i = 0; i < row+1; i++)
    {
        ...
        for (int j = 0; j < column; j++)
        {
            ...
            if (row > 0)
            {
                ...
                if (sret<0)
                {
                    ERR_MS("server do_server_history_query send");
                    return;
                }
            }
        }
