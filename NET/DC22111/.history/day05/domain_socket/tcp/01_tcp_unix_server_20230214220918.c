...
int main(int argc, char const *argv[])
{
   ...
   strcpy(sun.sun_path,"./unix");
    ...
    //必须绑定 bind: 功能 将IP地址和端口号绑定到指定套接字中
    if(bind(sfd, (const struct sockaddr *)&sun,sizeof(sun)) < 0)
    {
        ERR_MSG("bind");
        return -1;
    }
    ...
    struct sockaddr_un cin; //存储客户端的地址信息
