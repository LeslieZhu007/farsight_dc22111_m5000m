...
int main(int argc, char const *argv[])
{
    ...
    socklen_t optlen1 = sizeof(recv_timeout);
    if(getsockopt(sfd,SOL_SOCKET,SO_RCVTIMEO,&recv_timeout,&optlen1)< 0)
    {
        perror("setsockopt");
        return -1;
    }
    ...
    socklen_t optlen2 = sizeof(send_timeout);
    if(getsockopt(sfd,SOL_SOCKET,SO_SNDTIMEO,&send_timeout,&optlen2)< 0)
    {
        perror("setsockopt");
        return -1;
    }
