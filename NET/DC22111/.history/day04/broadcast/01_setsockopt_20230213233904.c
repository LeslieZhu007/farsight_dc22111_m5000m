...
int main(int argc, char const *argv[])
{
    ...
    if(getsockopt(sfd,SOL_SOCKET,SO_RCVTIMEO,&recv_timeout,&(sizeof(recv_timeout)))< 0)
    {
        perror("setsockopt");
        return -1;
    }
    ...
    if(getsockopt(sfd,SOL_SOCKET,SO_SNDTIMEO,&send_timeout,&(sizeof(send_timeout)))< 0)
    {
        perror("setsockopt");
        return -1;
    }
