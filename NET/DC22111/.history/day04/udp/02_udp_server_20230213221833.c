...
int main(int argc, char const *argv[])
{
    ...
    //connect将需要通信的对端信息地址记录到内核
    //此时当前UDP只能与该记录的对端通信
    struct sockaddr_in peer_in;
    peer_in.sin_family = AF_INET;
    peer_in.sin_port = htons(3333);
    peer_in.sin_addr.s_addr = inet_addr("192.168.8.177");

    if (connect(sfd,(struct sockaddr *)&peer_in,sizeof(peer_in)) < 0)
    {
        ERR_MSG("connect");
        return -1;
    }
