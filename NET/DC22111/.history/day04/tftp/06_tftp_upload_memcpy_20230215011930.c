...
int main(int argc, char const *argv[])
{
    ...
    *p1 = htons(2);  //上传
    char *p2 = buf + 2;
    strcpy(p2,argv[1]); //文件名 ==>终端拿 1_udpSer.c
    ...
    *p5 = 0;
    ...
    ssize_t ret = 0;
    ssize_t ret1 = 0;
    int fd = open(argv[1],O_RDONLY); //只读不需权限
    if (fd < 0)
    {
        ERR_MSG("open");
        return -1;
    ...
    while (1)
    {
        ...
        */
        ...
        //从文件中读取数据，开始向服务器发送
        ret1 = read(fd,send_buf,sizeof(send_buf));  //二进制文件内容的拷贝不能用strcpy  strcpy(buf+4,sendbuf)=>错误
        /*
        测试
        ...
        *((short *)buf+1) = htons(num);    //块编号
        num++;
        //拷贝
        memcpy(buf+4,send_buf,ret1);
        ...
        */
        ...
        if(sendto(sfd,buf,ret1+4,0,(struct sockaddr *)&sin,addrlen) < 0)
        {
            ERR_MSG("sendto");
            return -1;
        }
