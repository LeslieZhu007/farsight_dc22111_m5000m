...
int main(int argc, char const *argv[])
{
    ...
    struct sockaddr_in cin_array[getdtablesize()-4]; //0,1,2,sfd 不会有对应的客户端，所以-4
    ...
    while (1)
    {
        ...
        for (int i = 0; i <=maxfd; i++)
        {
            ...
            } else
            {
                ...
                {
                    printf("[%s | %d] newfd = %d 客户端断开连接\n",inet_ntoa(cin_array[i-4].sin_addr),ntohs(cin_array[i-4].sin_port),i);
                    //断开连接需关闭文件描述符
                    close(i);
                    //从readfds中剔除
                    FD_CLR(i,&readfds);
                ...
                printf("[%s | %d]newfd = %d:%s\n",inet_ntoa(cin_array[i-4].sin_addr),ntohs(cin_array[i-4].sin_port),i,buf);
