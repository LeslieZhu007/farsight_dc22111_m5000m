...
int main(int argc, char const *argv[])
{
    ...
    cin.sin_port = htons(SER_PORT); //服务器绑定的端口号
    cin.sin_addr.s_addr = inet_addr(SER_IP); //服务器绑定的IP
    bind(cfd,(struct sockaddr *)&cin,0)


    //填充要连接的服务器的ip和端口
    struct sockaddr_in sin;
    sin.sin_family = AF_INET; //必须填AF_INET
    sin.sin_port = htons(CLI_PORT); //客户端绑定的端口号
    sin.sin_addr.s_addr = inet_addr(CLI_IP); //客户端绑定的IP
