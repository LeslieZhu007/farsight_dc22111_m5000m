...
void do_admin_modification(int sockfd,MSG *msg)//管理员修改
{
	...
	*/
	...
	/*
		fscanf输入数值时不必做空值校验，因为有%d的格式限制符
		所以直到输入数字。否则fscanf不会结束
	*/
	printf("请输入年龄>>>");
...
void do_user_query(int sockfd,MSG *msg)
{
	...
	if(0!=strcmp(msg->info.name,name))
	{
		printf("不允许查询非本人的信息!!\n");
		return;
	}
...
void do_user_modification(int sockfd,MSG *msg)
{
	...
	strcpy(msg1.username,name);
	strcpy(msg1.info.name,msg->info.name);
	//2.发送查询请求给服务器
	send(sockfd, &msg1, sizeof(MSG), 0);
	//3.循环接受服务器响应
	while (1)
	...
	fscanf(stdin,"%d",&(msg->info.usertype));
    while (getchar()!=10);
	if ((0!=msg->info.usertype) && (1!=msg->info.usertype))
	{
		printf("请输入正确的员工类型\n");
		return ;
	}
	//做限制，防止普通用户将自己权限修改为管理员
	if(1==msg->info.usertype)
	{
		printf("不可以将普通用户权限修改为管理员权限\n");
		return;
	}
