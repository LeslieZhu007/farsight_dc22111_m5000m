...
int sockfd;
char buf[128] = "";
int login_status = 0; //默认用户处于未登录状态，防止重复登录

int process_user_or_admin_login_request(int acceptfd,MSG *msg)
{
	printf("------------%s-----------%d.\n",__func__,__LINE__);
	//检查登录状态标识，如果为0，可以登录，如果登录置为1，如果为1，不可登录
	if (login_status > 0)
	{
		char *errmsg;
		char sql[DATALEN] = {0};
		...
		strcpy(msg->recvmsg,"请勿重复登录!\n");
		//插入登录失败历史记录
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,LOGIN_IN_REPEAT);
