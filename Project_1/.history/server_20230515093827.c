...
int process_user_modify_request(int acceptfd,MSG *msg)
{
	...
	sprintf(sql, USERINFO_UPDATE_FORMAT,staffno,usertype,name,passwd,age,\
										 phone,addr,work,date,level,salary,recvmsg);
...
int process_user_query_request(int acceptfd,MSG *msg)
{
	...
	sprintf(sql,USRINFO_QUERY_SQL,msg->info.name);
	if(sqlite3_get_table(db,sql,&result,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
		printf("---****----%s.\n",errmsg);
		get_system_time(time_buf);
...
int process_admin_adduser_request(int acceptfd,MSG *msg)
{
	...
	} else
	{
		...
		get_system_time(time_buf);
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_ADDUSER_SUCCESS);
		printf("server:process_admin_adduser_request historyinfo insert sql = %s \n",sql);
		if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
		{
			fprintf(stderr,"管理员添加用户信息成功历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
		}
		printf("%s====>管理员添加用户信息成功历史记录插入成功\n",msg->username);	
...
int process_admin_deluser_request(int acceptfd,MSG *msg)
{
	...
	int nrow,ncolumn;
	char sql[1024] = {0};
	sprintf(sql, USRINFO_DELETE_FORMAT,msg->info.name);
	...
	if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
	{
		...
		get_system_time(time_buf);
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_DELUSER_FAIL);
		printf("server:process_admin_deluser_request historyinfo insert sql = %s \n",sql);
		if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
		{
			fprintf(stderr,"管理员删除用户信息失败历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
		}
		printf("%s====>管理员删除用户信息失败历史记录插入成功\n",msg->username);	
	...
	} else
	{
		...
		get_system_time(time_buf);
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_DELUSER_SUCCESS);
		printf("server:process_admin_deluser_request historyinfo insert sql = %s \n",sql);
		if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
		{
			fprintf(stderr,"管理员删除用户信息成功历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
		}
		printf("%s====>管理员删除用户信息成功历史记录插入成功\n",msg->username);	
...
int process_admin_query_request(int acceptfd,MSG *msg)
{
	...
	*/
	...
	if(sqlite3_get_table(db,sql,&result,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
		printf("---****----%s.\n",errmsg);		
		get_system_time(time_buf);
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_QUERY_FAIL);
		printf("server:process_admin_query_request historyinfo insert sql = %s \n",sql);
		if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
		{
			fprintf(stderr,"管理员查询失败历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
		}
		printf("%s====>管理员查询失败历史记录插入成功\n",msg->username);	
	}else{
		printf("enter else...\n");
		if(nrow == 0){
			msg->flags = 0; //查询失败
			...
			printf("server user info query row = %d,column =%d\n",nrow,ncolumn);
			get_system_time(time_buf);
			sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_QUERY_SUCCESS);
			printf("server:process_admin_query_request historyinfo insert sql = %s \n",sql);
			if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
			{
				fprintf(stderr,"管理员查询成功历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
			}
			printf("%s====>管理员查询成功历史记录插入成功\n",msg->username);	
			int count = 0;
			//row中不包含表头的一行，但是结果中有表头的内容，所以需要将表头的一行加上
			for (int i = 0; i < nrow+1; i++)
			{
...
int process_admin_history_request(int acceptfd,MSG *msg)
{
	...
	sprintf(sql,HISTORYINFO_QUERY_FORMAT,msg->info.name);
	if(sqlite3_get_table(db,sql,&result,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
		printf("---****----%s.\n",errmsg);	
		get_system_time(time_buf);
		sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_HISTORY_FAIL);
		printf("server:process_admin_query_request historyinfo insert sql = %s \n",sql);
		if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
		{
			fprintf(stderr,"管理员查询历史记录失败历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
		}
		printf("%s====>管理员查询历史记录失败历史记录插入成功\n",msg->username);		
	}else{
		//printf("----nrow-----%d,ncolumn-----%d.\n",nrow,ncolumn);		
		if(nrow == 0){
			msg->flags = 0; //查询失败
			...
			printf("server user info query row = %d,column =%d\n",nrow,ncolumn);
			get_system_time(time_buf);
			sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,ADMIN_HISTORY_SUCCESS);
			printf("server:process_admin_query_request historyinfo insert sql = %s \n",sql);
			if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
			{
				fprintf(stderr,"管理员查询历史记录成功历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
			}
			printf("%s====>管理员查询历史记录成功历史记录插入成功\n",msg->username);
			int count = 0;
			//row中不包含表头的一行，但是结果中有表头的内容，所以需要将表头的一行加上
			for (int i = 0; i < nrow+1; i++)
			{
...
int process_client_quit_request(int acceptfd,MSG *msg)
{
	...
	if(sqlite3_get_table(db,sql,&result,&nrow,&ncolumn,&errmsg) != SQLITE_OK){
		...
		if(nrow == 0){
			...
			strcpy(msg->recvmsg,LOGIN_IN_STATUS_NOT_FOUND);
			get_system_time(time_buf);
			sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,QUIT_FAIL);
			printf("server:process_client_quit_request historyinfo insert sql = %s \n",sql);
			if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
			{
				fprintf(stderr,"用户退出失败历史记录插入失败,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
			}
			printf("%s====>用户退出失败历史记录插入成功\n",msg->username);
			send(acceptfd,msg,sizeof(MSG),0);
			return -1;
		}else{
				//未登录状态可以，登录
				...
				//插入登出成功历史记录
				get_system_time(time_buf);
				sprintf(sql,HISTORYINFO_INSERT_FORMAT,time_buf,msg->username,QUIT_SUCCESS);
				printf("historyinfo insert sql = %s\n",sql);
				if (sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK)
				{
					fprintf(stderr,"historyinfo insert history failed,__%d__,%d,%s\n",__LINE__,sqlite3_errcode(db),errmsg);
