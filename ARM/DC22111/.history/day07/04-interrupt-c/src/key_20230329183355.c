...
void hal_exti_init()
{
    ...
    /*********EXTI init*********/
    //1.外部中断选择寄存器  EXTI_EXTICRx   PF9引脚和EXTI9进行连接
...
void hal_gicd_init()
{
    //1 GICD control register (GICD_CTLR)  设置GICD层全局控制器使能
    GICD->CTRL|= (0x1<<0); //使能组0

    //2. GICD_ISENABLERx  设置GICD层中断使能寄存器
    ...
    //2. GICD_IPRIORITYR  设置GICD层中断优先级寄存器
    //key1 PF9 ==> EXTI9 ----> 事件9 --->中断号 99
    GICD->IPRIORITYR[24] |= (0x1<<27);  //中断优先级设置相同
    //或者置 0     GICD->IPRIORITYR[24] &= (~(0x1f<<27)); 
    ...
    //2. GICD_ITARGETSRx 设置GICD层中断目标分配寄存器
    //key1 PF9 ==> EXTI9 ----> 事件9 --->中断号 99
    GICD->ITARGETSR[24] |= (0x3<<24); //分配给cpu0  0bx1  0b01或 0b11  最好先清零
...
void hal_gicc_init()
{
    //GICC_CTLR寄存器    GICC全局控制寄存器使能 CTRL[0] = 1
    GICC->CTRL |= (0x1<<0);

    //GICC_PMR寄存器     PMR[7:3] = 11111
    GICC->PMR &= (~(0x1f<<3));   //需先将五位清零
    GICC->PMR |= (0x2<<4);  //  
}