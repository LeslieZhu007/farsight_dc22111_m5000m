...
{
    int X = arr[low];
    while (low < high) //使得继续下一趟排序继续进行
    {
        while (arr[high]>=X &&low < high) //此时的low<high防止排序过程中错位情况
        {
            high--;
        }
        arr[low] = arr[high]; //将小值前放
...
void quick_sort(int *arr,int low,int high)
{
    if (low < high)
    {
        ...
        quick_sort(arr,low,mid-1);  //左侧快排
        quick_sort(arr,mid+1,high); //右侧快排
    }
