...
//改
int dlinklist_modify_pos(dlinklist *head,int pos,data_t data)
{
    if (NULL == head || pos < 0 || pos > dlinklist_len(head) - 1)
    {
        return -1;
    }
...
//查
data_t dlinklist_show_pos(dlinklist *head,int pos)
{
    if (NULL == head || pos < 0 || pos > dlinklist_len(head) - 1)
    {
        return -1;
...
int main(int argc, char const *argv[])
{
    ...
    //删除2
    dlinklist_delete_pos(head,2);

    dlinklist_show(head);

    //查找index=2的数
    printf("index为%d的数为:%d\n",2,dlinklist_show_pos(head,2));

    //修改index为2的数据为999
    dlinklist_modify_pos(head,2,999);
    dlinklist_show(head);
