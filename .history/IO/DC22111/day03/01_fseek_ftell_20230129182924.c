...
int main(int argc, char const *argv[])
{
    ...
    //修改偏移量到文件结尾 ----> 加上ftell可以计算文件大小
    fseek(src_fp,0,SEEK_END); // ==  rewind(src_fp)
    long offset =ftell(src_fp); //offset即文件大小
    printf("offset = %ld\n",offset);
    ...
    int ret;
    //从开头继续往前偏
    fprintf(stdout,"从开头继续往前偏:");
    ret = fseek(src_fp,-3,SEEK_SET);
    offset =ftell(src_fp);
    printf("ret = %d\n",ret);
    printf("offset = %ld\n",offset);


    fprintf(stdout,"从末尾继续往后偏:");
    ret = fseek(src_fp,10,SEEK_END);
    printf("ret = %d\n",ret);
    offset =ftell(src_fp);
    printf("offset = %ld\n",offset);
