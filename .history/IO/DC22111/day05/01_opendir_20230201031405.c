...
void print_file_info_by_file_name(char *parentdir,char *filename)
{
    //printf("parentdir = %s\n",parentdir);
    char *readir = (char *)malloc(sizeof(parentdir) + sizeof(filename)-1);
    //拼接全路径
    strcat(readir,parentdir);
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if ('.' !=(rp->d_name)[0] ) // *(rp->d_name)
        {
            ...
            char *parentdir = NULL;
            parentdir = argv[1];
            print_file_info_by_file_name(parentdir,rp->d_name);
        }
    }
    /******************end***********************/
