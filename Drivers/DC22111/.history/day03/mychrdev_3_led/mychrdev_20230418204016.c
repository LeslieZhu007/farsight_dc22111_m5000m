...
static int __init mychrdev_init(void)
{
    ...
    if (major < 0)
    {
     ...
     //寄存器地址映射
     //LED1 moder odr GPIOE10 
     vir_moder1 = ioremap(GPIOE->MODER,sizeof(gpio_t));
     if (vir_moder1 == NULL)
     {
     ...
     vir_odr1 = ioremap(GPIOE->ODR,sizeof(gpio_t));
     if (vir_odr1 == NULL)
     {
         printk("ioremap failed:%d\n",__LINE__);
         return -ENOMEM;
    ...
    //LED3 moder odr GPIOF10
     vir_moder1 = ioremap(GPIOF->MODER,sizeof(gpio_t));
     if (vir_moder1 == NULL)
     {
         printk("ioremap failed:%d\n",__LINE__);
         return -ENOMEM;
     ...
     vir_odr1 = ioremap(GPIOF->ODR,sizeof(gpio_t));
     if (vir_odr1 == NULL)
     {
         printk("ioremap failed:%d\n",__LINE__);
         return -ENOMEM;
    ...
    vir_rcc = ioremap(RCC->MP_AHB4ENSETR,sizeof(rcc_t));
    if (vir_rcc == NULL)
    {
        printk("ioremap failed:%d\n",__LINE__);
        return -ENOMEM;
