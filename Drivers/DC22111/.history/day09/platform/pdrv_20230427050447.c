...
//底半部处理函数
void myirq_func(struct work_struct *work)
{   
    int i;
    printk("enter half bottom mechanism=>\n");
    for (i = 0; i < 50;i++) {
        if(i % 3 == 0)
        {
        ...
        if(i % 3 == 1)
        {
            gpiod_set_value(desc0,!gpiod_get_value(desc0));
        }
        if(i % 3 == 2)
        {
            gpiod_set_value(desc1,!gpiod_get_value(desc1));
        } 

    }
    printk("leave half bottom mechanism=>\n");
}
